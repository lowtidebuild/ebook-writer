...
def chapter_number_label(chapter_num: int, language: str) -> str:
    """Return the reader-facing chapter number label for the language."""
    if language == "ko":
        return f"제{chapter_num}장"
    return f"Chapter {chapter_num}"


def wrap_chapter(chapter_html: str, chapter_num: int, language: str = "en") -> str:
    ...
    if h1_match:
        ...
        title_text = chapter_number_label(chapter_num, language)
    ...
    return (
        ...
        f'    <span class="chapter-number">{chapter_number_label(chapter_num, language)}</span>\n'
...
def generate_bibliography(citations_path: str, language: str) -> str:
    """Generate bibliography HTML from citations.json."""
    ...
    if not data.get("citations"):
        return ""
    title = "참고문헌" if language == "ko" else "Bibliography"
    html = f'<div class="chapter bibliography"><h1>{title}</h1><ol>'

    def _citation_sort_key(citation: dict) -> tuple[int, int | str]:
        citation_id = citation.get("id", "")
        if isinstance(citation_id, int):
            return (0, citation_id)
        if isinstance(citation_id, str) and citation_id.isdigit():
            return (0, int(citation_id))
        return (1, str(citation_id))

    for citation in sorted(data["citations"], key=_citation_sort_key):
        author = citation.get("author", "")
        title_text = citation.get("title", "")
        url = citation.get("url", "")
        accessed = citation.get("accessed", "")
        entry = f'<li id="cite-{citation["id"]}">'
        if author:
            entry += f"{author}. "
        entry += f"<em>{title_text}</em>"
        if url:
            entry += f' — <a href="{url}">{url}</a>'
        if accessed:
            acc_label = "접근일" if language == "ko" else "accessed"
            entry += f" ({acc_label}: {accessed})"
        entry += "</li>"
        html += entry
    html += "</ol></div>"
    return html
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--citations",
        default=None,
        help="Path to citations.json for bibliography generation (optional).",
    )
    p.add_argument(
        "--generator-credit",
        default=None,
        help="Optional generator credit shown on the copyright page.",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    for idx, ch_html in enumerate(chapter_htmls):
        chapter_sections.append(wrap_chapter(ch_html, chapter_nums[idx], args.language))
    ...
    copyright_page_html = render_copyright_page(
        author=args.author,
        date_str=date_str,
        generator_credit=args.generator_credit,
    )
    ...
    # 10.5. Generate bibliography if citations.json provided.
    bibliography_html = ""
    if args.citations and os.path.exists(args.citations):
        bibliography_html = generate_bibliography(args.citations, args.language)
        if bibliography_html:
            print(f"Generated bibliography from {args.citations}")
